...
module.exports = {
    ...
    async run (client, message, args) {
        ...
        if (kullanıcı.roles.cache.get(panel['kadın-rol-1'])) return message.reply(`Bu kullanıcı zaten kayıtlı. ${x.x}`)
        if (kullanıcı.roles.cache.get(panel['kadın-rol-2'])) return;
        if (kullanıcı.roles.cache.get(panel['kadın-rol-3'])) return;

        let toplamkadın = db.fetch(`toplam_kadın_${message.author.id}`)
        db.add(`toplam_kadın_${message.author.id}`, +1)

        const embed = new MessageEmbed()
        .setDescription(`${kullanıcı}, başarıyla ${message.author} yetkilisi tarafından kayıt edildi. ${x.o}`)
        .addFields(
            { name: 'Toplam kadın kayıt sayısı:', value: toplamkadın, inline: true }
        )
        .setColor('RANDOM')
        .setFooter({ Text: 'Crew v13 alt yapı'})
        message.reply({ embeds : [embed] })
        kullanıcı.roles.add(panel['kadın-rol-1'])
        kullanıcı.roles.add(panel['kadın-rol-2'])
        kullanıcı.roles.add(panel['kadın-rol-3'])
